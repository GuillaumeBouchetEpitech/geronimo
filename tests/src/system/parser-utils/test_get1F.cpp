...
TEST(basic_regexp_parser, get1F) {
...

  {
    ...
    try {
      ...
      ASSERT_NE(expectionMsg.find(D_SSTR("cannot parse 1F, type=get1F, toSearch=\"" << toSearch << "\"")), std::string_view::npos) << "expectionMsg=" << expectionMsg;
    }
  }

  {
    ...
    try {
      ...
      const std::string_view expectionMsg = err.what();
      ASSERT_NE(expectionMsg, "unreachable") << "expectionMsg=" << expectionMsg;
      ASSERT_NE(expectionMsg.find(D_SSTR("value of 1F is too low, type=get1F, toSearch=\"" << toSearch << "\"" << ", value=-666.00, minValue=-601.00")), std::string_view::npos) << "expectionMsg=" << expectionMsg;
    }
  }

  {
    ...
    try {
      ...
      const std::string_view expectionMsg = err.what();
      ASSERT_NE(expectionMsg, "unreachable") << "expectionMsg=" << expectionMsg;
      ASSERT_NE(expectionMsg.find(D_SSTR("value of 1F is too high, type=get1F, toSearch=\"" << toSearch << "\"" << ", value=666.00, maxValue=602.00")), std::string_view::npos) << "expectionMsg=" << expectionMsg;
    }
  }
}
