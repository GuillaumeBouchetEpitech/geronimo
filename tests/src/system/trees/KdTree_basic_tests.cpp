...
TEST(trees_KdTree, must_find_correct_number_of_object) {

  struct TmpUserData {
    int32_t id;
  };

  using myKdTree = gero::trees::KDTree<TmpUserData>;
...
TEST(trees_KdTree, must_find_correct_object_in_range) {
...
// TEST(trees_KdTree, basic_tests) {

//   gero::trees::KDTree::Vec3Data allPoints;
//   allPoints.reserve(2048);
...
//   // int32_t totalMilliSeconds2 = 0;
//   // int32_t minMilliSeconds2 = 0;
//   // int32_t maxMilliSeconds2 = 0;

//   constexpr int totalSteps = 30;

//   for (int kkk = 0; kkk < totalSteps; ++kkk)
//   {
//     // tmpClock.start();
...
//     {
//       found.clear();
//       myKDTree.searchWithRadius(toSearchPoint, 20.0f, found);
...
//     if (savedTotalFound < 0) {
//       savedTotalFound = totalFound;
//     }

//     // tmpClock.stop();
//     // const int32_t stepMilliSeconds2 = tmpClock.getDuration();

//     // minMilliSeconds2 = std::min(minMilliSeconds2, stepMilliSeconds2);
//     // maxMilliSeconds2 = std::max(maxMilliSeconds2, stepMilliSeconds2);

//     // totalMilliSeconds2 += stepMilliSeconds2;
//   }
...
//     found.clear();
//     myKDTree.searchWithRadius(toSearchPoint, k_radius, found);
...
//       const float magnitude = glm::distance(fPoint.position, toSearchPoint);
...
//       const float magnitude = glm::distance(toConfirmPoint, toSearchPoint);
...
//     if (totalTestFound > int(found.size())) {
//       std::cerr << "totalTestFound " << totalTestFound << std::endl;
//       std::cerr << "int(found.size()) " << int(found.size()) << std::endl;
//       throw std::runtime_error("missing some data");
//     }
