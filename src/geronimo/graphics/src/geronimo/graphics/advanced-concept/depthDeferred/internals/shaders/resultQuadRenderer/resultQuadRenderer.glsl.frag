...
void main(void)
{
  ...
  else
  {
    ...
    // if (diffuseCoef > startCoef)
    // {
      float extraCoef = diffuseCoef - startCoef;
      startColor += tmpDiffuseColor.rgb * max(0.0, extraCoef);
    // }

    // vec3 spotLightColor = tmpDiffuseColor.rgb * (max(diffuseCoef, startCoef) - min(diffuseCoef, startCoef));
    // vec3 diffuseColor = startColor + spotLightColor;
