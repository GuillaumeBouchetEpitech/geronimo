...
class DepthDeferredNoSPecular {
public:
  DepthDeferredNoSPecular() = default;
  ~DepthDeferredNoSPecular() = default;
...
public:
  ...
  void renderHudQuad(const gero::graphics::camera::ICamera& inSceneCamera, const gero::graphics::camera::ICamera& inHudCamera);

public:
  float getResolutionScaling() const;

private:
  depthDeferredNoSpecular::ScreenRecorder _screenRecorder;
  depthDeferredNoSpecular::ScreenRecorder _screenRecorder_transparency;

  depthDeferredNoSpecular::LightStackRenderer _lightStackRenderer;

  depthDeferredNoSpecular::ResultQuadRenderer _resultQuadRenderer;

  depthDeferredNoSpecular::FrameMerger _frameMerger;
