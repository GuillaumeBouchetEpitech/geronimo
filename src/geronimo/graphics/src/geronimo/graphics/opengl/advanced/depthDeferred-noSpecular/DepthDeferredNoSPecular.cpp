...
#include "DepthDeferredNoSPecular.hpp"
...
void DepthDeferredNoSPecular::resize(const glm::ivec2& inFrameSize, float inResolutionScaling /*= 1.0f*/) {
...
// void DepthDeferredNoSPecular::setEyePosition(const glm::vec3& inEyePos) { _eyePos = inEyePos; }

void DepthDeferredNoSPecular::setSunLightDirection(const glm::vec3& inSunLightDirection) {
  _sunLight = glm::vec4(glm::normalize(glm::vec3(inSunLightDirection)), 0.0f);
}
void DepthDeferredNoSPecular::setSunLightPosition(const glm::vec3& inSunLightPosition) {
  _sunLight = glm::vec4(inSunLightPosition, 1.0f);
}

void DepthDeferredNoSPecular::setAmbientLightRatio(float ambientLightRatio) { _ambientLightRatio = ambientLightRatio; }
...
void DepthDeferredNoSPecular::renderHudQuad(const gero::graphics::camera::ICamera& inSceneCamera,
                                            const gero::graphics::camera::ICamera& inHudCamera) {
...
float DepthDeferredNoSPecular::getResolutionScaling() const { return _resolutionScaling; }
