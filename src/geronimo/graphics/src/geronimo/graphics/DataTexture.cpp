...
void DataTexture::update(const glm::vec4* inDataPointer,
                         uint32_t inDataLength,
                         const glm::uvec2& inOriginSize,
                         const glm::uvec2& inUpdateSize) {
  _validateUpdate(inDataLength, inOriginSize, inUpdateSize);

  _texture.updateRgbaFloat(inOriginSize, inUpdateSize, inDataPointer);
}

void DataTexture::update(const std::vector<glm::vec4>& data,
                         const glm::uvec2& inOriginSize,
                         const glm::uvec2& inUpdateSize) {
...
void DataTexture::update(const float* inDataPointer,
                         uint32_t inDataLength,
                         const glm::uvec2& inOriginSize,
                         const glm::uvec2& inUpdateSize) {
  _validateUpdate(inDataLength, inOriginSize, inUpdateSize);
...
void DataTexture::update(const std::vector<float>& data,
                         const glm::uvec2& inOriginSize,
                         const glm::uvec2& inUpdateSize) {
  update(data.data(), uint32_t(data.size()), inOriginSize, inUpdateSize);
}
...
void DataTexture::update(const int16_t* inDataPointer,
                         uint32_t inDataLength,
                         const glm::uvec2& inOriginSize,
                         const glm::uvec2& inUpdateSize) {
  _validateUpdate(inDataLength, inOriginSize, inUpdateSize);
...
void DataTexture::update(const std::vector<int16_t>& data,
                         const glm::uvec2& inOriginSize,
                         const glm::uvec2& inUpdateSize) {
  update(data.data(), uint32_t(data.size()), inOriginSize, inUpdateSize);
}
...
void DataTexture::update(const int32_t* inDataPointer,
                         uint32_t inDataLength,
                         const glm::uvec2& inOriginSize,
                         const glm::uvec2& inUpdateSize) {
  _validateUpdate(inDataLength, inOriginSize, inUpdateSize);
...
void DataTexture::update(const std::vector<int32_t>& data,
                         const glm::uvec2& inOriginSize,
                         const glm::uvec2& inUpdateSize) {
  update(data.data(), uint32_t(data.size()), inOriginSize, inUpdateSize);
}
...
void DataTexture::_validateUpdate(uint32_t inDataLength,
                                  const glm::uvec2& inOriginSize,
                                  const glm::uvec2& inUpdateSize) const {
  ...
  if (int32_t(inOriginSize.x) > k_maxTextureWidth)
    D_THROW(std::runtime_error,
            "data texture origin x too high enough, input: " << inOriginSize.x << ", max: " << k_maxTextureWidth);
  if (int32_t(inOriginSize.y) > k_maxTextureWidth)
    D_THROW(std::runtime_error,
            "data texture origin y too high enough, input: " << inOriginSize.y << ", max: " << k_maxTextureWidth);
  ...
  if (int32_t(inOriginSize.x + inUpdateSize.x) > k_maxTextureWidth)
    D_THROW(std::runtime_error,
            "data texture width too high enough, input: " << (inOriginSize.x + inUpdateSize.x)
                                                          << ", max: " << k_maxTextureWidth);
  if (int32_t(inOriginSize.y + inUpdateSize.y) > k_maxTextureWidth)
    D_THROW(std::runtime_error,
            "data texture height too high enough, input: " << (inOriginSize.y + inUpdateSize.y)
                                                           << ", max: " << k_maxTextureWidth);
  ...
  if (inDataLength > 0 && inUpdateSize.y > 1) {
    ...
    if (divideDiff != inUpdateSize.y || moduloDiff != 0U)
      D_THROW(std::runtime_error,
              "data texture size not matching the texture size, data length: " << inDataLength
                                                                               << ", texture size: " << inUpdateSize);
  }
}
