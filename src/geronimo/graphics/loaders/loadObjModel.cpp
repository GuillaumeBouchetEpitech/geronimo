...
#include "geronimo/helpers/internals/TinyObjLoader.hpp"
...
void loadObjModel(const std::string& filename, const std::string& mtlDir, ModelVertices& vertices) {
  ...
  for (std::size_t shape = 0; shape < shapes.size(); ++shape) {
    ...
    for (std::size_t face = 0; face < faceVertices.size(); ++face) {
      ...
      //     glm::vec3 newNormal = glm::normalize(glm::cross(vertex2 - vertex3,
      //     vertex3 - vertex1));
      //     // glm::vec3 newNormal = glm::normalize(glm::cross(vertex2 -
      //     vertex1, vertex3 - vertex1));
