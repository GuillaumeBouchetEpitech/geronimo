...
SimplexNoise::SimplexNoise(float frequency /*= 1.0f*/,
                           ...
                           int seed /*= 0*/)
  : _frequency(frequency), _amplitude(amplitude), _lacunarity(lacunarity), _persistence(persistence) {
  ...
  std::vector<uint8_t> bufferIn; // TODO: use dynamic_heap_array
  bufferIn.reserve(256);
  for (std::size_t ii = 0; ii < perm.size(); ++ii)
    bufferIn.push_back(perm[ii]);

  std::vector<uint8_t> bufferOut;
  bufferOut.reserve(256);

  rng::DeterministicRng rng;
  rng.setSeed(seed);

  while (!bufferIn.empty()) {
    // move one (deterministic) random value from bufferIn to bufferOut
    const int index = rng.getValue() % int(bufferIn.size());
    bufferOut.push_back(bufferIn[std::size_t(index)]);

    // fast removal
    std::swap(bufferIn[std::size_t(index)], bufferIn.back());
    bufferIn.pop_back();
  }

  for (std::size_t ii = 0; ii < bufferOut.size(); ++ii)
    _perm[ii] = bufferOut[ii];
}
 ...
 */
uint8_t SimplexNoise::hash(int32_t i) const { return _perm[static_cast<uint8_t>(i)]; }
...
static float grad(int32_t hash, float x) {
  ...
  float grad = 1.0f + float(h & 7); // Gradient value 1.0, 2.0, ..., 8.0
  if ((h & 8) != 0)
...
float SimplexNoise::noise(float x, float y) const {
  ...
  const float x1 = x0 - float(i1) + G2; // Offsets for middle corner in (x,y) unskewed coords
