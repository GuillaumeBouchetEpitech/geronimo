...
void ConvexPolygon::preAllocate(std::size_t size) { _vertices.reserve(size); }

void ConvexPolygon::addVertex(const glm::vec2& pos) { _vertices.push_back(pos); }
...
const std::vector<glm::vec2>& ConvexPolygon::getVertices() const { return _vertices; }
