...
PhysicConeTwistConstraint::PhysicConeTwistConstraint(const PhysicConeTwistConstraintDef& def) {
  ...
  _bullet.constraint =
    new btConeTwistConstraint(*(def.body_a->getRawRigidBody()), *(def.body_b->getRawRigidBody()), frameInA, frameInB);

  _bullet.constraint->setDbgDrawSize(1.0f);
}

PhysicConeTwistConstraint::~PhysicConeTwistConstraint() { delete _bullet.constraint; }
...
void PhysicConeTwistConstraint::setAngularOnly(bool angularOnly) { _bullet.constraint->setAngularOnly(angularOnly); }

bool PhysicConeTwistConstraint::getAngularOnly() const { return _bullet.constraint->getAngularOnly(); }

void PhysicConeTwistConstraint::setLimit(float _swingSpan1,
                                         float _swingSpan2,
                                         float _twistSpan,
                                         float _softness /*= 1.f*/,
...
float PhysicConeTwistConstraint::getDamping() const { return _bullet.constraint->getDamping(); }

void PhysicConeTwistConstraint::setDamping(float damping) { _bullet.constraint->setDamping(damping); }
