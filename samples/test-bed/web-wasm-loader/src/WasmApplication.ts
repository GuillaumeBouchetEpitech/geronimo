...
export class WasmApplication {
  ...
  private _wasmApplicationStartFunc: (width: number, height: number) => void;
  private _wasmApplicationUpdateFunc: (deltaTime: number, isMouseLocked: number) => void;
  ...
  async initialize(
    ...
    await this._setupWasmApplication(inLogger);
  }
  ...
  private async _loadWasmApplication(
    ...
    // @ts-ignore
    this._module = await testBedApp(moduleArgs);
  ...
  private async _setupWasmApplication(
    ...
    this._wasmApplicationStartFunc(this._width, this._height);
  }
