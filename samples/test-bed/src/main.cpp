...
void validateInputs(uint32_t inWidth, uint32_t inHeight) {
...
void processCommandLineArgs(Application::Definition& def, int argc, char** argv) {
  if (argc <= 1) {
    return;
  }
...
int main(int argc, char** argv) {
  ...
  myApplication.setMouseLockStatus(true);
  myApplication.run();
