...
void TrianglesStackRenderer::initialize(ShaderAliases shaderId, GeometryIds geometryId) {
...
void TrianglesStackRenderer::setMatricesData(const gero::graphics::Camera::MatricesData& matricesData) {
  _matricesData = matricesData;
}

std::size_t TrianglesStackRenderer::getTrianglesCount() const { return _vertices.size(); }
...
void TrianglesStackRenderer::pushCircle(const glm::vec2& center, float radius, const glm::vec4& color, float z) {
  ...
  for (std::size_t ii = 0; ii < quality; ++ii) {
    const float coef = float(ii) / quality;

    cachedValues.at(ii).x = std::cos(coef * gero::math::pi2);
    cachedValues.at(ii).y = std::sin(coef * gero::math::pi2);
  }

  for (std::size_t ii = 0; ii < quality; ++ii) {
    const std::size_t indexA = ii;
    const std::size_t indexB = (ii + 1) % quality;

    const glm::vec3 vertexA = {
      center.x + radius * cachedValues.at(indexA).x, center.y + radius * cachedValues.at(indexA).y, z};

    const glm::vec3 vertexB = {
      center.x + radius * cachedValues.at(indexB).x, center.y + radius * cachedValues.at(indexB).y, z};
