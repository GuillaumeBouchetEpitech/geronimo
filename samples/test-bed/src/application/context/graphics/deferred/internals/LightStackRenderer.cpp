...
void LightStackRenderer::flush(const glm::vec3& eyePos,
  ...
  _flushDiffuse(composedMatrix, positionTexture, normalTexture, depthTexture);

  _specular.frameBuffer.bind();

  GlContext::clears(GlContext::Buffers::depth);

  _flushSpecular(eyePos, composedMatrix, colorTexture, positionTexture, normalTexture, _diffuse.depthTexture);
