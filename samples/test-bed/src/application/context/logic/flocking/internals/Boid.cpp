...
Boid::strafe(const glm::vec3& target, float inHorizontalAngle, float inVerticalAngle, float coef) {
  ...
  const float tmpHorizontalAngle = gero::math::getAngle(currentDirection.x, currentDirection.y);
  const float tmpVerticalAngle =
    gero::math::getAngle(glm::length(glm::vec2(currentDirection.x, currentDirection.y)), currentDirection.z);
  const float newHorizontalAngle = tmpHorizontalAngle + inHorizontalAngle;
  const float newVerticalAngle = tmpVerticalAngle + inVerticalAngle;
