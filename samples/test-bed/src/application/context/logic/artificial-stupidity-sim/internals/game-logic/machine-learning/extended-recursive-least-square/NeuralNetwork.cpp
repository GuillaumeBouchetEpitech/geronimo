...
//     output *= 0.1f;
//   }
//   return gero::math::clamp(output, -10.0f, 10.0f);
// }
...
namespace AllActivations {
  ...
  namespace sigmoid {
    ...
    float initWeight()
    {
      // const float rawValue = gero::rng::RandomNumberGenerator::getRangedValue(0.05f, 0.10f);
      // // const float rawValue = gero::rng::RandomNumberGenerator::getRangedValue(0.5f, 0.9f);
      // const bool isNegative = gero::rng::RandomNumberGenerator::getRangedValue(0.0f, 1.0f) < 0.5f;
      // // const float newValue = rawValue * (isNegative ? -1.0f : 1.0f);
      // return rawValue * (isNegative ? -1.0f : 1.0f);
      return gero::rng::RandomNumberGenerator::getRangedValue(-0.2f, 0.2f);
    }
  };
  ...
  namespace linear {
    float activate(float x) {
      // return x;
      return gero::math::clamp(x, -3.0f, 3.0f);
    }
    float derive(float x) {
      return 1.0f;
    }
    float initWeight()
    {
      const float rawValue = gero::rng::RandomNumberGenerator::getRangedValue(0.50f, 0.99f);
      const bool isNegative = gero::rng::RandomNumberGenerator::getRangedValue(0.0f, 1.0f) < 0.5f;
      return rawValue * (isNegative ? -1.0f : 1.0f);
    }
  };
  ...
  namespace ReLU {
    float activate(float x) {
      ...
      // return gero::math::clamp(x, 0.0f, 3.0f);
    }
    float derive(float x) {
    ...
    float initWeight()
    {
      // const float rawValue = gero::rng::RandomNumberGenerator::getRangedValue(0.01f, 0.05f);
      // const bool isNegative = gero::rng::RandomNumberGenerator::getRangedValue(0.0f, 1.0f) < 0.5f;
      // return rawValue * (isNegative ? -1.0f : 1.0f);
      // // return rawValue;
      return gero::rng::RandomNumberGenerator::getRangedValue(-0.2f, 0.2f);
  ...
  namespace leakyReLU {
    float activate(float x) {
      ...
      return gero::math::clamp((x < 0.0f) ? 0.1f * x : x, -3.0f, 3.0f);
    }
    float derive(float x) {
    ...
    float initWeight()
    {
      const float rawValue = gero::rng::RandomNumberGenerator::getRangedValue(0.05f, 0.10f);
      // const float rawValue = gero::rng::RandomNumberGenerator::getRangedValue(0.5f, 0.9f);
      const bool isNegative = gero::rng::RandomNumberGenerator::getRangedValue(-1.0f, 1.0f) < 0.0f;
      // const float newValue = rawValue * (isNegative ? -1.0f : 1.0f);
      return rawValue * (isNegative ? -1.0f : 1.0f);
      // this->_inputSynapseWeights.push_back(newValue);
      // this->_inputSynapseWeights.push_back(gero::rng::RandomNumberGenerator::getRangedValue(0.5f, 0.9f));
    }
  };
  ...
  {
    return s_asArray.at(gero::asValue(type));
  }
...
NeuralNetworkNeuron::NeuralNetworkNeuron(std::size_t numInputs, NeuralNetworkActivations activationType)
{
  ...
  for (std::size_t ii = 0; ii < numInputs; ++ii) {
    ...
    // // this->_inputSynapseWeights.push_back(gero::rng::RandomNumberGenerator::getRangedValue(-0.5f, 0.5f));
  ...
  // // this->_biasSynapseWeight = gero::rng::RandomNumberGenerator::getRangedValue(-0.5f, 0.5f);
  ...
  // // this->_biasSynapseWeight = gero::rng::RandomNumberGenerator::getRangedValue(0.5f, 0.9f);
...
void NeuralNetworkNeuron::updateInputSynapseWeights(std::size_t ii, float value) const
{
  ...
  this->_inputSynapseWeights.at(ii) = gero::math::clamp(this->_inputSynapseWeights.at(ii), -k_weightRange, k_weightRange);
}

void NeuralNetworkNeuron::updateBiasSynapseWeight(float value) const
{
  this->_biasSynapseWeight += value;
  this->_biasSynapseWeight = gero::math::clamp(this->_biasSynapseWeight, -k_weightRange, k_weightRange);
}
...
NeuralNetwork::NeuralNetwork(const NeuralNetworkTopology& topology)
  : _topology(topology)
{
  gero::rng::RandomNumberGenerator::ensureRandomSeed();
