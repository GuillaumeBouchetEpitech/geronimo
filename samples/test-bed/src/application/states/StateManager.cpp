...
void StateManager::handleEvent(const SDL_Event& event) { _states[gero::asValue(_currentState)]->handleEvent(event); }

void StateManager::update(uint32_t delta) { _states[gero::asValue(_currentState)]->update(delta); }

void StateManager::render(const SDL_Window& window) { _states[gero::asValue(_currentState)]->render(window); }

void StateManager::resize(uint32_t width, uint32_t height) {
  _states[gero::asValue(_currentState)]->resize(width, height);
}

void StateManager::visibility(bool visible) { _states[gero::asValue(_currentState)]->visibility(visible); }
